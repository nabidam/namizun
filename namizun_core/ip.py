...
def get_random_range_ip_from_database(db = database('localhost')):
    return choice(db.get_cache_parameter('range_ips').split('\n'))


def get_random_ip_from_database(db = database('localhost')):
    fragmented_random_ip = get_random_range_ip_from_database(db).split('.')
    if fragmented_random_ip[0].isdigit() and 0 < int(fragmented_random_ip[0]) < 255 and \
        ...
        return get_random_ip_from_database(db)
...
def cache_ip_ports_from_database(db = database('localhost')):
    global cache_ip_list
    cache_ip_list = db.get_ip_ports_from_database()


def get_random_ip_port(db = database('localhost')):
    if len(cache_ip_list) > 0:
        target_ip, target_port = choice(list(cache_ip_list.items()))
        del cache_ip_list[target_ip]
    else:
        target_ip = get_random_ip_from_database(db)
        target_port = get_game_port()
        db.set_ip_port_to_database(target_ip, target_port)
