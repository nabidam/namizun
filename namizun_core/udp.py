...
def start_udp_uploader(db = database('localhost')):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    target_ip, game_port = ip.get_random_ip_port(db)
    ...
    while remain_upload_size >= 0:
        selected_buffer_range = choices(buffer_ranges, db.buffers_weight, k=1)[0]
        ...
        if sock.sendto(bytes(buf), (target_ip, game_port)):
            ...
            sleep(0.001 * int(uniform(5, 26)) / db.get_cache_parameter('coefficient_buffer_sending_speed'))
    sock.close()
...
def set_upload_size_and_uploader_count(total_upload_size, total_uploader_count, db = database('localhost')):
    global total_upload_size_for_each_ip, uploader_count
    uploader_count = int(uniform(total_uploader_count * 0.05, total_uploader_count * 0.2))
    coefficient_of_upload = int((db.get_cache_parameter('coefficient_buffer_size') + 1) / 2)
...
def multi_udp_uploader(total_upload_size, total_uploader_count, db = database('localhost')):
    set_upload_size_and_uploader_count(total_upload_size, total_uploader_count, db)
    threads = []
    store_new_upload_agent_log(uploader_count, total_upload_size_for_each_ip)
    for sender_agent in range(uploader_count):
        agent = Thread(target=start_udp_uploader, args=(db,))
        agent.start()
