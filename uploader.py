...
def reboot_finder():
    ...
    if new_upload_amount >= cached_upload_amount and new_download_amount >= cached_download_amount:
        db.set_parameter('total_download_cache', new_download_amount)
        db.set_parameter('total_upload_cache', new_upload_amount)
    else:
        system_upload_amount, system_download_amount = get_system_network_io()
        db.set_parameter('download_amount_synchronizer',
                         (cached_download_amount - system_download_amount))
        db.set_parameter('upload_amount_synchronizer',
                         (cached_upload_amount - system_upload_amount))
...
def get_network_usage():
    ...
    limitation = int(uniform(db.get_cache_parameter('coefficient_limitation') * 0.7,
                             db.get_cache_parameter('coefficient_limitation') * 1.3))
...
def get_uploader_count_base_timeline():
    ...
    default_uploader_count = db.get_cache_parameter(
        'coefficient_uploader_threads_count') * 10
